...
// 座標計算機
function calculateCoords(lat, lon, respawnLat, respawnLon, pre = 3, exp = 3) {
  // 指定有効数字 pre で切り捨て
...
function getCoordsSettings() {
  ...
  if (isNaN(exp)) exp = parseInt(localStorage.getItem("coord-exp"), 10) || 3;
  return { pre, exp };
}
...
function distanceFromOrigin(lat, lon) {
  ...
  const a = Math.sin(dLat / 2) ** 2 +
            Math.cos(toRadians(lat1)) * Math.cos(toRadians(lat)) *
            Math.sin(dLon / 2) ** 2;
...
function updateCenterCoords() {
  ...
  if (respawnCoords) {
    ...
    text = `${distanceFromOrigin(dy, dx).toFixed(6)} km (${dy}, ${dx})`;
  } else {
